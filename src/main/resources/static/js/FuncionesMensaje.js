...
function itemSkate(){
    $.ajax({
        ...
        success:function(respuesta){
            ...
            mySelect += "<option value='null' id='select-skate'>Seleccionar Skate</option>"
...
function traerInformacionMensaje(){
    $.ajax({
        url:"http://144.22.57.76:8080/api/Message/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
...
function pintarRespuestaMensaje(respuesta){
    let myTable="<table><thead><tr><th>Nombre</th><th>skate</th><th>Comentario</th><th>Acciones</th></tr></thead>";
    for(i=0;i<respuesta.length;i++){
        myTable+="<tr>";
...
function guardarInformacionMensaje(){
    let var2 = {
        ...
        $.ajax({
            ...
            url:"http://144.22.57.76:8080/api/Message/save",
...
function borrarInformacionMensaje (idElemnto){
    ...
    $.ajax({
        ...
        url:"http://144.22.57.76:8080/api/Message/"+idElemnto,
        success:function(response){
            console.log(response);
...
function editarInformacionMensaje (idElemento){
    $.ajax({
        url:"http://144.22.57.76:8080/api/Message/"+idElemento,
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
            $("#id").val(respuesta.idMessage),
            $("#"+respuesta.client.idclient).attr("selected",true),
            $("#"+respuesta.skate.id).attr("selected",true),
...
function InformacionMensaje(){
    let var2 = {
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            data: JSON.stringify(var2),
            url:"http://144.22.57.76:8080/api/Message/update",
